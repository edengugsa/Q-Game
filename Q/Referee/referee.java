...
public class Referee {
  ...
  private Map<String, AIPlayer> players;
  private final List<String> disqualifiedPlayers;

  /**
   * FOR TESTING. Resumes a Game from a previous state.
   */
  public Referee(List<AIPlayer> players, GameState game) {
    if (players.size() != game.numPlayers()) {
      throw new IllegalStateException("Players and GameState aren't consistent");
    }
    this.game = game;
  ...
  public Referee(List<AIPlayer> players, QRuleBook ruleBook) {
    if (players.size() != game.numPlayers()) {
      throw new IllegalStateException("Players and GameState aren't consistent");
    }
...
//    renderedGame.repaint();
  ...
  private void tellPlayersGameResult(List<String> winners) {
    for (AIPlayer p : this.players.values()) {
...
//        winners.remove(p.name()); TODO: is a Player who throws in win() still considered a winner?
  ...
  /**
   * Sets this Referee's storage of AIPlayer into a name->player map using the given list of AIPlayer.
   */
  private List<String> setPlayers(List<AIPlayer> players) {
  ...
  /**
   * Builds this Referee's representation of the GameState
  ...
  /**
   * Disqualifies the current player. Goes to the next Player in GameState.
   */
